...
//        },
    ...
    screens.define_action_button({
        ...
        'condition': function () {
            return this.pos.config.iface_create_purchase_order;
        },
    });
    ...
    */
    var CreateSaleOrderScreenWidget = screens.ScreenWidget.extend({
        ...
        show: function(){
            ...
            if (!this.pos.config.iface_create_draft_purchase_order) {
                this.$('#button-create-draft-order').addClass('oe_hidden');
            }
            if (!this.pos.config.iface_create_confirmed_purchase_order) {
                this.$('#button-create-confirmed-order').addClass('oe_hidden');
            }
            if (!this.pos.config.iface_create_delivered_purchase_order) {
        ...
        click_sale_order_button: function(action) {
            ...
            this.gui.show_popup('confirm', {
                'title': _t(
                    'Create Purchase Order and discard the current' +
                    ' PoS Order?'),
                'body': _t(
                    "This operation will permanently discard the current PoS" +
                    " Order and create a Purchase Order, based on the" +
                    " current order lines."),
                confirm: function(){
                    framework.blockUI();
                    rpc.query({
